						...
						<tbody>
							...
							if ($result->num_rows > 0) {
								while($row = $result->fetch_assoc()) {
									if ($row['akses'] == 'Admin') {
										// remove master user from table list
									}else {
										echo '
										<tr class="c-table__row">
											...
											<td class="c-table__cell"><a class="c-btn c-btn--primary c-btn--small" id="updateuser" href="javascript:;" data-toggle="modal" data-target="#modalupdate" data-username="'.$row['username'].'" data-password="'.$row['password'].'" data-id="'.$row['id'].'">Update</a> 
												<a class="c-btn c-btn--danger c-btn--small" id="hapus" href="javascript:;" data-url="?module=user&aksi=hapus&identity='.$row['id'].'" data-toggle="modal" data-target="#modalhapus">Hapus</a></td></tr>
												';
											}
										}
									}
